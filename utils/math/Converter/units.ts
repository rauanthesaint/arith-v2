...
const convertTemperature = (value: number, fromUnit: string, toUnit: string): number => {
  ...
  const fahrenheitToKelvin = (fahrenheit: number): number => celsiusToKelvin(fahrenheitToCelsius(fahrenheit));
  const kelvinToFahrenheit = (kelvin: number): number => celsiusToFahrenheit(kelvinToCelsius(kelvin));
  ...
  switch (fromUnit) {
    ...
    case 'Kelvin':
      ...
      break;
    default:
      throw new Error(`Unknown temperature conversion: ${fromUnit} to ${toUnit}`);
  }
...
const convertUnits = (value: number, fromUnit: string, toUnit: string, unitType: 'length' | 'weight' | 'temperature' | 'time' | 'currency'): number => {
